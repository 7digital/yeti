...
var BrowserWaiter = require("./browserwaiter");
...
CLI.prototype.runBatch = function runBatch(options) {
    ...
    function prepareTests(client) {
        ...
        var browserWaiter = new BrowserWaiter(client);
        ...
        client.getAgents(function (err, agents) {
            ...
            if (agents.length > 0) {
                ...
                browserWaiter.wait({
                    ...
                    onTimeout: function failure() {
                        self.panic('\n>>> Timeout expired before the expected number of connections were established\n\n' +
