    ...
    YUI({
        ...
        driver.addAutomation("test", "mocha", {
            ...
            bindFn: function (win, undef) {
                ...
                win.mocha.options.reporter = 'mochaToYETIReporter';
         
                win.mocha._reporter = function myReporter(runner) {
         
                    // runner.ignoreLeaks = true;
         
                    runner.on('test end', function (test) {
                        var suiteName = test.title,
                            testPassed = (test.state === 'passed'),
                            testFailed = (test.state === 'failed');
         
                        tests[suiteName] = {
                            message: (testFailed && test.err) ? test.err.message : '',
                            result: testPassed || "fail",
                            name: suiteName
                        };
